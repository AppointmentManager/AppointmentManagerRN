...
export default function SignUpScreen() {
    ...
    const handleVerifyOtp = async () => {
        ...
        if (response.success && response.data) {
            // After OTP verification, navigate to profile setup for new users
            if (response.data.isNewUser) {
                navigation.replace('ProfileSetup', {
                    token: response.data.token,
                    userId: response.data.user.id,
                    phoneNo: inputMode === 'phone' ? trimmedIdentifier : undefined,
                    emailId: inputMode === 'email' ? trimmedIdentifier.toLowerCase() : undefined,
                });
            } else {
    ...
    const totalSteps = 3; // 1: Phone/Email, 2: OTP, 3: Profile (on next screen)
                ...
                >
                    ...
                    <View style={styles.titleSection}>
                        <Text style={styles.title}>
                            {otpSent ? 'Verify your identity' : 'Create an account'}
                        </Text>
                        <Text style={styles.subtitle}>
                            {otpSent
                                ? `Enter the 6-digit code sent to your ${inputMode === 'email' ? 'email' : 'phone'}`
                                : 'Start by verifying your phone number or email'}
                        </Text>
                    </View>
